// Anti-Link Bot v2.9.3 - Deterministic Owner Exempt + Session Persistence + Optimizations
...
let sessionPrinted = false; // Track if session has been printed
...
// Message queue for handling high spam
const messageQueue = [];
let queueProcessing = false;
let highSpamMode = false;
const QUEUE_DELAY_NORMAL = 50;
const QUEUE_DELAY_SPAM = 10; // Changed from 50ms to 10ms during high spam
...
function saveSessionToEnv() {
  try {
    ...
    if (sessionPrinted) return; // Don't print session multiple times
    ...
    sessionPrinted = true; // Mark as printed
    console.log("");
...
function addToQueue(msg, priority = false) {
  if (priority) {
    messageQueue.unshift(msg); // Add to front for priority processing
  } else {
    messageQueue.push(msg);
  }
  
  // Enable high spam mode if queue gets large
...
async function processQueue() {
  ...
  while (messageQueue.length > 0) {
    ...
    try {
      ...
      console.log("⚠️ Queue processing error:", e?.message);
    }
    
    // Dynamic delay based on spam mode
    const delay = highSpamMode ? QUEUE_DELAY_SPAM : QUEUE_DELAY_NORMAL;
...
async function startBot() {
  try {
    ...
    // Only request pairing code if not registered and no session restored
    if (!state.creds.registered && !sessionRestored) {
      ...
      console.log("✅ Using restored session - no pairing needed");
    }
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
        ...
        if (statusCode === DisconnectReason.loggedOut) {
          ...
          sessionPrinted = false; // Reset session print flag
        } else {
    ...
    async function handleMessage(msg) {
      try {
        ...
        if (hasLink) {
          console.log("⚡ IMMEDIATE LINK DETECTED - Priority processing");
          // Process immediately for links
          ...
          console.log("");
          console.log("🚫 LINK VIOLATION DETECTED (IMMEDIATE)");
        ...
        // ===== NOT OWNER - CHECK FOR OTHER VIOLATIONS =====
    ...
    sock.ev.on("messages.upsert", async (m) => {
      ...
      for (const msg of messages) {
        ...
        // Add to queue with priority if contains link
        addToQueue(msg, hasLink);
      }
    });
